...
# Check events
kubectl get events -n dclaw-hr --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-hr deployment/dclaw-hr-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-hr-db-credentials -n dclaw-hr
   ```
